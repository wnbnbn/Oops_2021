...
- 目录快速索引不再等待整个收件箱复制结束。收件箱有独立、进程内单实例工作线程；转移与其他文件写入共享公平锁，每个文件之间让出锁。
- 每个已校验归档文件安排独立叶节点入库，约 750 ms 合并一批，最多 16 项；不等待整个收件箱完成，不以增量任务清扫整目录。
- 媒体元数据分析改为 25 项小批次；新文件到达不重排正在播放的视频。隐藏相册在重新显示时刷新，卡牌大厅后面的主相册也不反复做隐藏列表差分。
- 卡牌长按预览的更多菜单提供信息、分享、外部打开、最近删除及永久删除。删除经 Activity Result 交给主界面持久任务队列；用户确认后退出未完成对局，已完成战绩不变。
- 选胜者点击范围限于实际卡片，不包括布局留白。视频播放核心、随机权重、卡牌升级条件与对局计分未改。

完整线程/数据流、断点恢复边界、测试范围与真机验收见 [V0.9.3](LocalFeed/docs/V0.9.3.md)。版本发布必须通过源码 SHA-256、纯 Kotlin 回归、Android 编译和固定证书检查；这些检查不代替真机速度、触控和解码验收。

## v0.9.4 原生闪卡试用版

图片更多菜单、卡牌冠军和战绩胜者可以进入独立闪卡展示页。效果由 Android 原生 Canvas 绘制，包含触控倾斜、彩虹反光、foil 斜纹、局部高光与回正动画。相册只显示静态金属边框，红心和等级文字已移除，避免多媒体网格持续动画或遮挡时长。

尺寸适配、入口、兼容范围与真机验收见 [V0.9.4](LocalFeed/docs/V0.9.4.md)。

## v0.9.5 闪卡可见性修复版

独立闪卡页现在对所有卡牌启用完整闪卡材质，并在没有触摸时自动缓慢扫光。手指按下后切换为实时跟随，松手回正后恢复自动扫光。卡牌大厅保持触摸驱动，避免持续动画影响对局性能。

修复范围与真机验收见 [V0.9.5](LocalFeed/docs/V0.9.5.md)。
